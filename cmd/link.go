...
var linkCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		name := args[0]
		if linkTarget == "" {
			return fmt.Errorf("--target is required (claude-code, openclaw, cursor, codex, opencode)")
		}
...
func init() {
	linkCmd.Flags().StringVar(&linkTarget, "target", "", "Target tool (claude-code, openclaw, cursor, codex, opencode)")
}
